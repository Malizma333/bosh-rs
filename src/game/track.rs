...
impl Track {
    ...
    pub fn entity_positions_at(&self, frame: usize) -> Vec<Entity> {
        ...
        if let Some(riders) = position_cache.get(frame) {
            ...
            for i in len..=frame {
                if DEBUG_PRINT { println!("Frame {}", i); }
...
mod tests {
    ...
    fn zero_lines() {
      ...
      assert!(avg_position1.0 > avg_position0.0, "Rider should have increased x");
      assert!(avg_position1.1 > avg_position0.1, "Rider should have increased y");
    }

    #[test]
    fn one_line() {
      let y = 5.0;
      let line = &Line::builder().id(0).line_type(LineType::Normal).point(0.0, 5.0).point(30.0, 5.0).build();
      let frame = 11;
      ...
      assert!(rider_colliding_points.1 < rider_falling_points.1, "Rider should have collided with line");
      assert!(rider_colliding_points.1 < y - 0.01, "Rider should not have been flattened");
    }

    #[test]
    fn two_lines() {
      let y = 5.0;
      let line1 = Line::builder().id(0).line_type(LineType::Normal).point(0.0, 5.0).point(30.0, 5.0).build();
      let line2 = Line::builder().id(1).line_type(LineType::Normal).point(-7.0, 0.0).point(-7.0, 10.0).flipped(true).build();
      let frame = 15;
      ...
      let sled = entities.get(1).expect("Sled should exist as separate entity");
      ...
      println!("Shoulder ({}, {})", shoulder.location.0, shoulder.location.1);
      println!("Nose ({}, {})", shoulder.location.0, shoulder.location.1);
      println!("Butt ({}, {})", butt.location.0, butt.location.1);
      assert!(shoulder.location.0 > 0.0 && nose.location.0 < 0.0, "Rider should have been separated from sled");
      assert!(shoulder.location.1 < y - 0.01, "Rider should not have been flattened");
      assert!(butt.location.1 > y - 0.01 && butt.location.0 > 0.0 && shoulder.location.0 > butt.location.0, "Rider should be sitting and leaning forward");
    }
